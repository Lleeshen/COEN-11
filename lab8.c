...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		printf("To add to wait list, type 1. To remove a  waitlist, type 2. To show the wait list, type 3. To show waitlists under a group size, type 4. To change the size of a wait list by name (WARNING: moves you to end of wait list), type 5. To show every list backward, type 6. To show every name backward, type 7. To quit, type 0.\n");
		fflush(stdin);
		scanf("%c",&option);
		switch(option)
		{
			...
			case '0':
				...
				return 0;
			default:
				printf("Please enter 1, 2, 3, 4, 5, 6, 7 or 0.\n");
		}
	}
}
...
void showNameBackward(int i, SLOT* p, char *ptr)
{
	...
	if(p->next == NULL && i !=3 && *ptr == '\0')
	{
		for(j=i+1;j<4;j++)
		{
			if(waitl[j].head != NULL)
			{
				showNameBackward(j,waitl[j].head,waitl[j].head->names);
				putchar('\n');
				break;
			}
		}
